...
{
    std::string sbuf = std::string("hjpvsh trhfye: ") + to_string(newscreenwidth) + "*" + to_string(newscreenheight);
    if (((float)newscreenwidth <= (float)newscreenheight * 1.61) && ((float)newscreenwidth >= (float)newscreenheight * 1.59)) {
        sbuf += " (ibhjrjajhvfn)";
    }
    setText(0, sbuf);
    setText(14, fullscreen ? "gjdybq trhfy: nfr" : "gjdybq trhfy: ys");
    if (newdetail == 0) {
        setText(1, "ltnfkspfwsz: ybpmrj");
    }
    if (newdetail == 1) {
        setText(1, "ltnfkspfwsz: cthtlymj");
    }
    if (newdetail == 2) {
        setText(1, "ltnfkspfwsz: dbcjrf");
    }
    if (bloodtoggle == 0) {
        setText(2, "rhjd: ys");
    }
    if (bloodtoggle == 1) {
        setText(2, "rhjd: nfr, vtyi ltnfkmyj");
    }
    if (bloodtoggle == 2) {
        setText(2, "rhjd: nfr, ltnfkmyj");
    }
    setText(4, ismotionblur ? "hjpvbnnz: nfr" : "hjpvbnnz: ys");
    setText(5, decalstoggle ? "ltrfks: nfr (edsvrytyj)" : "ltrfks: ys");
    setText(6, musictoggle ? "vepbrf: nfr" : "vepbrf: ys");
    setText(9, invertmouse ? "sydthnedfnb vbie: nfr" : "sydthnedfnb vbie: ys");
    setText(10, std::string("xenkbdscnm: ") + to_string(int(usermousesensitivity * 5)));
    setText(11, std::string("uexyscnm: ") + to_string(int(volume * 100)));
    setText(13, showdamagebar ? "gjkjcf ehjye: nfr" : "gjkjcf ehjye: ys");
    if ((newdetail == detail) && (newscreenheight == (int)screenheight) && (newscreenwidth == (int)screenwidth)) {
        setText(8, "yfpfl");
    } else {
        setText(8, "yfpfl (ltzrs pvsyb gjnht,e&nm gthtpfdfynf;tyyz uhb)");
    }
}

void Menu::updateStereoConfigMenu()
{
    setText(0, std::string("edsvryenb cnthtj : ") + StereoModeName(newstereomode));
    setText(1, std::string("gjlskbnb cnthtj: ") + to_string(stereoseparation));
    setText(2, std::string("cnthtj htdthc: ") + (stereoreverse ? "Yes" : "No"));
}
...
void Menu::Load()
{
    ...
    switch (mainmenu) {
        ...
        case 3:
            ...
            addButton(13, "", 30, 125);
            addButton(7, "-yfkfinedfnb eghfdksyyz-", 10 + 15, 90);
            addButton(12, "-yfkfinedfnb pder-", 10 + 15, 55);
            addButton(8, "yfpfl", 10, 10);
            updateSettingsMenu();
            break;
        case 4:
            addButton(0, "", 10, 400);
            ...
            addButton(devtools ? 10 : 9, "yfpfl", 10, 10);
            updateControlsMenu();
            break;
        case 5: {
            LoadCampaign();
            addLabel(-1, Account::active().getName(), 5, 400);
            addButton(1, "nenjhsfk", 5, 300);
            addButton(2, "dbghj,edfyyz", 5, 240);
            addButton(3, "dblfkbnb ghjaskm", 400, 10);
            addButton(4, "ujkjdyt vty.", 5, 10);
            addButton(5, "pvsybnb ghjaskm", 5, 180);
            addButton(6, "rfvgfysz : " + Account::active().getCurrentCampaign(), 200, 420);
        ...
        case 6:
            addLabel(-1, "db dgtdytys oj ,f;f'nt dblfkbnb ghjaskm?", 10, 400);
            addButton(1, "nfr", 10, 360);
            addButton(2, "ys", 10, 320);
            break;
        case 7:
            if (Account::getNbAccounts() < 8) {
                addButton(0, "yjdbq ghjaskm", 10, 400);
            } else {
                addLabel(0, "ytvf ;jlyjuj ghjask%", 10, 400);
            }
            addLabel(-2, "", 20, 400);
            addButton(Account::getNbAccounts() + 1, "yfpfl", 10, 10);
            for (int i = 0; i < Account::getNbAccounts(); i++) {
                addButton(i + 1, Account::get(i).getName(), 10, 340 - 20 * (i + 1));
            }
            break;
        case 8:
            addButton(0, "kturbq", 10, 400);
            addButton(1, "crkflybq", 10, 360);
            addButton(2, ",j;tdskmybq", 10, 320);
            break;
        case 9:
            ...
            addButton(-1, "             htrjhl      rhfobq xfc", 10, 440);
            addButton(numchallengelevels, "yfpfl", 10, 10);
            break;
        case 10: {
            addLabel(0, campaignEndText[0], 80, 330);
            addLabel(1, campaignEndText[1], 80, 300);
            addLabel(2, campaignEndText[2], 80, 270);
            addButton(3, "yfpfl", 10, 10);
            addLabel(4, string("ndsq hf+eyjr:         ") + to_string((int)Account::active().getCampaignScore()), 190, 200);
            addLabel(5, string("htrjhl:      ") + to_string((int)Account::active().getCampaignHighScore()), 190, 180);
        } break;
        case 18:
            addButton(0, "", 70, 400);
            addButton(1, "", 10, 360);
